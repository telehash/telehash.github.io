...
The leader must ensure that the returned JSON encoding is always consistent between changes so that participants can use the SipHash digest calculated from the raw response to detect versions in the chat channel.
...
* **join** - (required) the join id of the sender (must match the roster)
